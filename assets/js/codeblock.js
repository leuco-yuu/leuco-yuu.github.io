...
function isFoldable(block) {
  return block.dataset.codeFoldable === "true";
}

function getToggleButtons(block) {
  return [
    ...block.querySelectorAll(".collapse-code-btn[data-code-action='toggle-collapse']"),
  ];
}

function updateCollapseButtons(block, collapsed) {
  const foldable = isFoldable(block);

  getToggleButtons(block).forEach((button) => {
    const isBottomButton = button.classList.contains("code-block-bottom-collapse");
    ...
    button.hidden = !foldable || (isBottomButton && collapsed);
    button.classList.toggle("hidden", !foldable || (isBottomButton && collapsed));
    button.classList.toggle("flex", foldable && !(isBottomButton && collapsed));
...
function setCollapsed(block, collapsed, { animate = true } = {}) {
  ...
  block.dataset.collapsed = String(nextCollapsed);
  content.style.transition = animate ? "max-height 0.3s ease-out" : "";
  content.style.maxHeight = nextCollapsed ? `${collapsedHeight}px` : "";
  content.style.overflow = nextCollapsed ? "hidden" : "";

  updateCollapseButtons(block, nextCollapsed);
  updateCollapseOverlay(block, nextCollapsed);
...
function initCollapsibleBlocks(root = document) {
  getCodeBlocks(root).forEach((block) => {
    if (block.dataset.collapsible !== "true") {
      block.dataset.codeFoldable = "false";
      return;
    }

    const defaultState = block.dataset.defaultState || "expanded";
    const collapsedAttr = block.dataset.collapsed === "true";
    const autoCollapsed = shouldAutoCollapse(block);
    const foldable = collapsedAttr || autoCollapsed;
    const startCollapsed = foldable && (collapsedAttr || defaultState === "collapsed" || autoCollapsed);

    block.dataset.codeFoldable = String(foldable);
...
function handleCodeBlockClick(event) {
  ...
  if (collapseButton) {
    ...
    if (!block || !isFoldable(block)) return;
